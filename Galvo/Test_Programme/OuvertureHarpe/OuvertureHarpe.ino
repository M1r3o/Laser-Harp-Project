...
int boutonClose = 7;     // Bouton fermeture
...
void loop() {
  ...
  while (digitalRead(boutonOpen)==HIGH  ) {
  ...
  for (int ecart=0; ecart < (4095/(nbCordes-1)); ecart++) {
    // PLACER LE GALVO
  ...
  for (int ecart=4095/(nbCordes-1); ecart > 0; ecart--) {
    // PLACER LE GALVO
    // On boucle sur toutes les cordes pour placer chacune
    for (int cordeCourante=0; cordeCourante < (nbCordes-1); cordeCourante++) {
      // Eteindre le laser

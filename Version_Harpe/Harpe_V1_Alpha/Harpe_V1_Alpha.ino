    ...
    for (int cordeCourante = 0; cordeCourante < (nbCordes - 1); cordeCourante++) {
      ...
      if (digitalRead(pinSensor) == true) {
        ...
        if (Notes[cordeCourante][FLAGON] == 0) {
          ...
          // On memorise que cette corde joue
          Notes[cordeCourante][FLAGON] = 1;
        }
        else {   // pas de lumiere detectee pour cette corde
          // Si la note etait en train de jouer
          if (Notes[cordeCourante][FLAGON] == 1) {
            // Il faut arreter de jouer la note (MIDI)
